...
var getValueFromEvent = function getValueFromEvent(event, valueState) {
  ...
  ? getValueForCheckbox(valueState, checked, value) : !!multiple // <select multiple>
...
function FormikConsumer(_ref) {
  ...
  return createElement(Fragment, null, children(formik));
}
function useFormikContextSelector(selector) {
...
function useFormik(_ref) {
  ...
  var getFieldHelpers = useEventCallback(function (name) {
    return {
  ...
  var getValueFromEvent = useEventCallback(function (event, fieldName) {
    // React Native/Expo Web/maybe other render envs
    if (!isReactNative && event.nativeEvent && event.nativeEvent.text !== undefined) {
      return event.nativeEvent.text;
    } // React Native
    ...
    var type = target.type,
        ...
        multiple = target.multiple;
    return /checkbox/.test(type) // checkboxes
    ? getValueForCheckbox(getIn(state.values, fieldName), checked, value) : !!multiple // <select multiple>
    ? getSelectedValues(options) : value;
  });
  var getFieldProps = useCallback(function (nameOrOptions) {
    var isAnObject = isObject(nameOrOptions);
    var name = isAnObject ? nameOrOptions.name ? nameOrOptions.name : nameOrOptions.id : nameOrOptions;
    var valueState = getIn(state.values, name);
    var touchedState = getIn(state.touched, name);
    var field = {
      name: name,
      value: valueState,
    ...
    if (isAnObject) {
      var type = nameOrOptions.type,
          ...
          _nameOrOptions$parse = nameOrOptions.parse,
          parse = _nameOrOptions$parse === void 0 ? /number|range/.test(type) ? numberParseFn : defaultParseFn : _nameOrOptions$parse,
          _nameOrOptions$format = nameOrOptions.format,
          format = _nameOrOptions$format === void 0 ? defaultFormatFn : _nameOrOptions$format,
          _nameOrOptions$format2 = nameOrOptions.formatOnBlur,
          formatOnBlur = _nameOrOptions$format2 === void 0 ? false : _nameOrOptions$format2;
      ...
      if (type !== 'radio' && type !== 'checkbox') {
        field.onChange = function (eventOrValue) {
    ...
    return field;
  }, [getValueFromEvent, handleBlur, handleChange, setFieldValue, state.touched, state.values]);
...
function useSetFieldValue() {
  return useFormikContextSelector(function (ctx) {
...
function useSetFieldTouched() {
  return useFormikContextSelector(function (ctx) {
    return ctx.setFieldTouched;
  });
}
function useField(nameOrOptions) {
  var isAnObject = isObject(nameOrOptions); // Normalize propsOrFieldName to FieldConfig<Value>

  var props = isAnObject ? nameOrOptions : {
    name: nameOrOptions
  };
  var fieldName = props.name,
      validateFn = props.validate;
  var registerField = useFormikContextSelector(function (c) {
    return c.registerField;
  });
  var unregisterField = useFormikContextSelector(function (c) {
    return c.unregisterField;
  });
  useEffect(function () {
    ...
    !registerField ? process.env.NODE_ENV !== "production" ? invariant(false, 'useField() / <Field /> must be used underneath a <Formik> component or withFormik() higher order component') : invariant(false) : void 0;
  ...
  var meta = useFieldMeta(fieldName);
  var valueState = meta.value,
      touchedState = meta.touched;
  var setFieldValue = useSetFieldValue();
  var setFieldTouched = useSetFieldTouched();
  var getFieldHelpers = useFormikContextSelector(function (ctx) {
    return ctx.getFieldHelpers;
  });
  var field = {
    name: fieldName,
    value: valueState,
    ...
    onChange: function onChange(eventOrValue) {
      if (isInputEvent(eventOrValue)) {
        ...
        setFieldValue(fieldName, parse(getValueFromEvent(eventOrValue, valueState), fieldName));
      } else {
  ...
  var type = nameOrOptions.type,
      ...
      multiple = nameOrOptions.multiple,
      _nameOrOptions$parse = nameOrOptions.parse,
      parse = _nameOrOptions$parse === void 0 ? /number|range/.test(type != null ? type : '') ? numberParseFn : defaultParseFn : _nameOrOptions$parse,
      _nameOrOptions$format = nameOrOptions.format,
      format = _nameOrOptions$format === void 0 ? defaultFormatFn : _nameOrOptions$format,
      _nameOrOptions$format2 = nameOrOptions.formatOnBlur,
      formatOnBlur = _nameOrOptions$format2 === void 0 ? false : _nameOrOptions$format2; // @todo why is this type failing?
...
function useFieldValue(name) {
  var state = useFormikContextSelector(function (ctx) {
    return getIn(ctx.values, name);
  });
  var set = useSetFieldValue();
  var setState = useCallback(function (value, shouldValidate) {
...
function useFieldError(name) {
  var state = useFormikContextSelector(function (ctx) {
    return getIn(ctx.errors, name);
  });
  var set = useFormikContextSelector(function (ctx) {
    return ctx.setFieldError;
  });
  var setState = useCallback(function (error) {
    set(name, error);
  }, [name]);
  return [state, setState];
}
...
function useFieldTouched(name) {
  var state = useFormikContextSelector(function (ctx) {
    return Boolean(getIn(ctx.touched, name));
  });
  var set = useSetFieldTouched();
  var setState = useCallback(function (isTouched, shouldValidate) {
...
function useFieldInitialValue(name) {
  return useFormikContextSelector(function (ctx) {
    return getIn(ctx.initialValues, name);
  });
}
/**
...
function useFieldInitialTouched(name) {
  return useFormikContextSelector(function (ctx) {
    return Boolean(getIn(ctx.initialTouched, name));
  });
}
/**
...
function useFieldInitialError(name) {
  return useFormikContextSelector(function (ctx) {
    return getIn(ctx.initialErrors, name);
  });
}
/**
...
function useInitialTouched() {
  return useFormikContextSelector(function (ctx) {
    return ctx.initialTouched;
  });
}
/**
...
function useErrors() {
  ...
  var update = useFormikContextSelector(function (ctx) {
    return ctx.setErrors;
  });
  return [state, update];
}
/**
...
function useValidateField(fieldName) {
  var validateField = useFormikContextSelector(function (ctx) {
...
function useFieldMeta(name) {
  var _useFieldValue = useFieldValue(name),
      value = _useFieldValue[0];

  var _useFieldTouched = useFieldTouched(name),
      touched = _useFieldTouched[0];

  var _useFieldError = useFieldError(name),
      error = _useFieldError[0];

  var initialValue = useFieldInitialValue(name);
  var initialTouched = useFieldInitialTouched(name);
  var initialError = useFieldInitialError(name);
  return {
    value: value,
    touched: touched,
    error: error,
...
function Field(_ref) {
  var render = _ref.render,
      ...
      props = _objectWithoutPropertiesLoose(_ref, ["render", "children", "as", "component"]);

  useEffect(function () {
    !!render ? process.env.NODE_ENV !== "production" ? invariant(false, "<Field render> has been deprecated. Please use a child callback function instead: <Field name={" + name + "}>{props => ...}</Field> instead.") : invariant(false) : void 0;
    !!(component && render) ? process.env.NODE_ENV !== "production" ? invariant(false, 'You should not use <Field component> and <Field render> in the same <Field> component; <Field component> will be ignored') : invariant(false) : void 0;
    ...
    !!(render && children && !isEmptyChildren(children)) ? process.env.NODE_ENV !== "production" ? invariant(false, 'You should not use <Field render> and <Field children> in the same <Field> component; <FastField children> will be ignored') : invariant(false) : void 0; // eslint-disable-next-line
  }, []);
  ...
  if (render) {
    ...
    return isFunction(render) ? render(_extends({}, legacyBag, {
      meta: meta
    })) : null;
  }
  ...
  if (typeof asElement === 'string') {
    var innerRef = props.innerRef,
        _rest = _objectWithoutPropertiesLoose(props, ["innerRef", "parse", "format", "formatOnBlur"]);
  ...
  var rest = _objectWithoutPropertiesLoose(props, ["parse", "format", "formatOnBlur"]);

  return createElement(asElement, _extends({}, field, rest), children);
}
...
var Form = /*#__PURE__*/forwardRef(function Form(props, ref) {
  ...
  var handleReset = useContextSelector(FormikContext, function (ctx) {
    return ctx.handleReset;
  });
  var handleSubmit = useContextSelector(FormikContext, function (ctx) {
...
function connect(Comp) {
  var C = function C(props) {
    return createElement(FormikConsumer, {
...
function ErrorMessage(_ref) {
  var component = _ref.component,
      ...
      children = _ref.children,
      name = _ref.name,
      rest = _objectWithoutPropertiesLoose(_ref, ["component", "render", "children", "name"]);

  var _useFieldTouched = useFieldTouched(name),
      touch = _useFieldTouched[0];
...
function FastField(_ref) {
  var render = _ref.render,
      children = _ref.children,
      is = _ref.as,
      component = _ref.component,
      shouldUpdate = _ref.shouldUpdate,
      props = _objectWithoutPropertiesLoose(_ref, ["render", "children", "as", "component", "shouldUpdate"]);
  ...
  useEffect(function () {
    ...
    !!shouldUpdate ? process.env.NODE_ENV !== "production" ? invariant(false, "<FastField shouldUpdate> has been deprecated. Please create a custom component with useField and React.memo() instead.") : invariant(false) : void 0; // eslint-disable-next-line
  }, []);
  ...
  var innerRef = props.innerRef,
      rest = _objectWithoutPropertiesLoose(props, ["innerRef", "parse", "format", "formatOnBlur", "validate"]);
...
export { ErrorMessage, FastField, Field, FieldArray, Form, Formik, FormikConsumer, FormikContext, FormikProvider, connect, defaultFormatFn, defaultParseFn, getActiveElement, getIn, getSelectedValues, getValueForCheckbox, getValueFromEvent, insert, isEmptyArray, isEmptyChildren, isFunction, isInputEvent, isInteger, isNaN$1 as isNaN, isObject, isPromise, isReactNative, isString, move, numberParseFn, prepareDataForValidation, replace, setIn, setNestedObjectValues, swap, useErrors, useField, useFieldError, useFieldInitialError, useFieldInitialTouched, useFieldInitialValue, useFieldTouched, useFieldValue, useFormik, useFormikContext, useFormikContextSelector, useInitialErrors, useInitialStatus, useInitialTouched, useInitialValues, useIsDirty, useIsSubmitting, useIsValid, useResetForm, useSetErrors, useSetFieldTouched, useSetFieldValue, useSetStatus, useSetTouched, useSetValues, useStatus, useSubmitForm, useTouched, useValidateField, useValidateForm, useValues, validateYupSchema, withFormik, yupToFormErrors };
